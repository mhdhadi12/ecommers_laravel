...
@section('content')
    <div class="main-content">
                                ...
                                <table class="table table-striped mb-0">
                                    ...
                                    @foreach ($rec as $value)
                                        <tr>
                                            ...
                                            <td>
                                                @foreach (explode(',', $value->foto) as $image)
                                                    <img src="{{ url('path/produk/' . $image) }}" alt=""
                                                        style="width : 100%">
                                                @endforeach
                                            </td>
                                            ...
                                            <td>
                                                <button class="btn btn-primary btn-action" data-toggle="modal"
                                                    data-target="#exampleModalCenter-{{ $value->id }}"><i
                                                        class="fas fa-pencil-alt"></i></button>

                                                <form method="POST" action="{{ url('/admin', $value->id) }}"
        ...
        @endphp
        @foreach ($rec as $value)
            <div class="modal fade" id="exampleModalCenter-{{ $value->id }}" tabindex="-1" role="dialog"
                aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalCenterTitle">Edit Data Ke - {{ $value->id }}</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <div class="modal-body">
                            <form action="{{ url('admin', $value->id) }}" method="post" enctype="multipart/form-data">
                                @csrf
                                {{ method_field('PUT') }}
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="kode" name="kode"
                                        value="{{ isset($value->kode) ? $value->kode : '' }}">
                                </div>
                                <div class="form-group">
                                    <label for="nama">Nama</label>
                                    <input type="text" class="form-control" id="nama" name="nama"
                                        value="{{ isset($value->nama) ? $value->nama : '' }}">
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="desc" name="desc"
                                        value="{{ isset($value->deskripsi) ? $value->deskripsi : '' }}">
                                </div>
                                <div class="form-group">
                                    <label for="saldoakhir">Stok Akhir</label>
                                    <input type="number" class="form-control" id="saldoakhir" name="saldoakhir"
                                        value="{{ isset($value->saldoakhir) ? $value->saldoakhir : '' }}">
                                </div>
